...
public class MatchController {
  ...
  public void initMatchScene(Hero hero) {
    ...
    Image monsterImage =
        new Image(String.valueOf(Controller.class.getResource("images/monster.png")));
  ...
  public void updateHealth() {
    ...
    monsterHealthIndicator_uiProgressBar.setProgress(
        (double) monsterHealth / (double) monsterMaxHealth);
  }
  ...
  public void runCycle(PlayerAction playerAction) throws InterruptedException {
    ...
    pause1.setOnFinished(
        event1 -> {
          updateHealth();
          // If the health of the hero or monster is 0, the winner is displayed, else the monster
          // attacks
          if (hero.getHealth() == 0 || monster.getHealth() == 0) {
            ...
            pause2.setOnFinished(
                event2 -> {
                  ...
                  updateHealth();
                  // If the health of the hero or monster is 0, the winner is displayed, else enable
                  // ui
                  if (hero.getHealth() == 0 || monster.getHealth() == 0) {
                    LOGGER.log(Level.INFO, "Heros or monsters health is 0!");
